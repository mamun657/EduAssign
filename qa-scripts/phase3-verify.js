// E2E QA: Phase 3 — Admin Dashboard live verification
...
(async () => {
  ...
  const report = {
    ...
    viewports: {},
    crud: { subjects: null },
    teacher: { login: null, dashboard: null },
    student: { login: null, dashboard: null },
    finishedAt: null,
    pass: false,
    error: null,
  };
    ...
    for (const vp of VIEWPORTS) {
      ...
      if (login.ok) {
        for (const route of ADMIN_ROUTES) {
          ...
          const shotPath = path.join(SHOTS_DIR, 'phase3_' + slugify(route) + '_' + vp.name + '.png');
